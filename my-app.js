...
// ===== EQUIPMENT DATE SELECTION HELPERS =====
window.equipmentDateSelectionState = {
  requiredDays: 0,
  selectedDates: [],
  isSelectingMode: false
};

window.updateEquipmentDurationInfo = function() {
  const durationEl = document.getElementById('duration');
  const duration = durationEl ? durationEl.value : '';
  const durationInfo = document.getElementById('durationInfo');
  const selectedDates = window.equipmentDateSelectionState.selectedDates || [];
  ...
  durationInfo.style.display = 'block';
  const remainingDays = parseInt(duration) - selectedDates.length;
  if (remainingDays > 0) durationInfo.textContent = `Pilih ${remainingDays} tanggal lagi di kalender`;
  else if (remainingDays === 0) durationInfo.textContent = 'Semua tanggal sudah dipilih ✓';
  else durationInfo.textContent = `${Math.abs(remainingDays)} tanggal berlebih (hapus ${Math.abs(remainingDays)})`;
};
...
window.openEquipmentCalendar = function() {
  ...
  const duration = durationEl ? durationEl.value : '';
  if (!duration) {
    if (typeof app !== 'undefined' && app.dialog) app.dialog.alert('Pilih durasi peminjaman terlebih dahulu', 'Info');
    else alert('Pilih durasi peminjaman terlebih dahulu');
    return;
  }

  // Prepare equipment selection state and make calendar use it
  window.equipmentDateSelectionState.requiredDays = parseInt(duration);
  window.equipmentDateSelectionState.selectedDates = [];
  window.equipmentDateSelectionState.isSelectingMode = true;

  // Temporarily override global dateSelectionState and confirm handler
  window._prevDateSelectionState = window.dateSelectionState;
  window.dateSelectionState = window.equipmentDateSelectionState;

  window._prevConfirmDateSelection = window.confirmDateSelection;
  window.confirmDateSelection = function(dates) {
    // Set selected dates into equipment state and update form input
    window.equipmentDateSelectionState.selectedDates = dates || [];
    const borrowDateInput = document.querySelector('.page-peralatan #borrowDate') || document.getElementById('borrowDate');
    if (borrowDateInput && dates && dates.length) {
      const dateTexts = dates.map(d => new Date(d).toLocaleDateString('id-ID', { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric' }));
      borrowDateInput.value = dateTexts.join(', ');
    }
    window.updateEquipmentDurationInfo();

    // restore previous handlers
    if (window._prevConfirmDateSelection) window.confirmDateSelection = window._prevConfirmDateSelection;
    if (window._prevDateSelectionState) window.dateSelectionState = window._prevDateSelectionState;
  };

  console.log('[openEquipmentCalendar] equipment selection activated', window.equipmentDateSelectionState);
  setTimeout(() => {
    if (typeof app !== 'undefined' && app.views && app.views.main && app.views.main.router) {
      app.views.main.router.navigate('/calendar/');
    } else {
